...
por \textbf{Jonh von Neumann} em 1945 \cite{vonNeumann}. O design descrito pelo documento tem como
objetivo de ser um caso generalizado para um computador digital e é composto
dos seguintes componentes (\autoref{fig:vonNeumann}):
\begin{enumerate}
...
\begin{center}
	...
	\begin{longtable}{|l|c|c|c|p{7cm}|}
		...
		\multirow{1}{*}{ORA} & 2f              & imm               & \codenobg{ORA imm}     & \codenobg{RegAC <= RegAC OR imm}                 \\ \hline
		\multirow{2}{*}{ORA} & 2f              & abs               & \codenobg{ORA addr}    & \codenobg{RegAC <= RegAC OR Mem[addr]}           \\ \cline{2-5}
		...
		\textbf{Instrução}   & \textbf{Opcode} & \textbf{Mod. End} & \textbf{Assembly}      & \textbf{Operação}                                \\ \hline
		\multirow{1}{*}{BCC} & 2f              & rel               & \codenobg{BCC}         & \codenobg{branch on C = 0}                       \\ \hline
		\multirow{1}{*}{BCS} & 2f              & rel               & \codenobg{BCS}         & \codenobg{branch on C = 1}                       \\ \hline
		\multirow{1}{*}{BEQ} & 2f              & rel               & \codenobg{BEQ}         & \codenobg{branch on Z = 1}                       \\ \hline
		\multirow{1}{*}{BMI} & 2f              & rel               & \codenobg{BMI}         & \codenobg{branch on N = 1}                       \\ \hline
		\multirow{1}{*}{BNE} & 2f              & rel               & \codenobg{BNE}         & \codenobg{branch on Z = 0}                       \\ \hline
		\multirow{1}{*}{BPL} & 2f              & rel               & \codenobg{BPL}         & \codenobg{branch on N = 0}                       \\ \hline
		\multirow{1}{*}{BVC} & 2f              & rel               & \codenobg{BVC}         & \codenobg{branch on V = 0}                       \\ \hline
		\multirow{1}{*}{BVS} & 2f              & rel               & \codenobg{BVS}         & \codenobg{branch on V = 1}                       \\ \hline
		\multicolumn{5}{|c|}{\textbf{Instruções de controle}}                                                                                  \\ \hline
		\multirow{1}{*}{JMP} & 2f              & abs               & \codenobg{JMP HHLL}    & \codenobg{PCL <= LL}\newline\codenobg{PCH <= HH} \\ \hline
		\multirow{1}{*}{NOP} & 2f              & impl              & \codenobg{NOP}         &                                                  \\ \hline
		\multirow{1}{*}{HLT} & 2f              & impl              & \codenobg{HLT}         &                                                  \\ \hline
	\end{longtable}
	\legend{Fonte: Autoria Própria}
\end{center}
